...
$("#dogdiv").load("parse_dognzb2db.php");
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    {
		$("#mytrailersdog").innerHTML="Please wait....";
        $("#mytrailersdog").load("addtocart.php",$("#dotorg2" + movieid2).serialize()); 
    }		
					...
					<tr valign="top">  					
						<td width=30px bgcolor=white height=76>	
							<?php
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM dogbzb";									
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM dogbzb";
									$result = $conn->query($sql);											
									if ($result->num_rows > 0) {
										echo "<a href={$DogNzbLandingPage} target=hrnew><img style=\"margin-top:5px; margin-left:3px;\" src=\"{$DogNzbLogoImage}\" >";
									}									
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$hrmovietitle2 = wordwrap($hrmovietitle2, 12, "\n", true);	
			...
			//	$addmycart = "https://api.dognzb.cr/api?t=cartadd&id=0e0db0297101048cd655b660eb514485&apikey=028b2b283bc277538f9136bd9f986ab1";
				$addmycart = "https://api.dognzb.cr/api?t=cartadd&id={$myguid}&apikey=ddad3c1052911760474e5b72cd27bef6";
				
				$addtocartbutton = "<form action='JavaScript:addmovietomycart($myimdb)' method='POST' id='dotorg2{$myimdb}' >
