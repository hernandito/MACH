					...
					<tr valign="top">  					
						<td width=30px bgcolor=white height=76>	
							<?php
									...
									$timeresult = mysql_query('SELECT timestamp from nzbcat');		
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM nzbsdotorg";	dog								
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM nzbcat";
									$result = $conn->query($sql);											
									if ($result->num_rows > 0) {
										echo "<a href={$NZBcatLandingPage} target=hrnew><img style=\"margin-top:5px; width: 28px; margin-left:3px;\" src=\"{$NZBcatLogoImage}\" >";
									}										
