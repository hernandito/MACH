    ...
    function addmovietomycart2(movieid2)
    {
					...
					<tr valign="top">  					
						<td width=30px bgcolor=white height=76>	
							<?php
									...
									$timeresult = mysql_query('SELECT timestamp from nzbfinder');		
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM nzbsdotorg";	dog								
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM nzbfinder";
									$result = $conn->query($sql);											
									if ($result->num_rows > 0) {
										echo "<a href={$NZBfinderLandingPage} target=hrnew><img style=\"margin-top:5px; width: 28px; margin-left:3px;\" src=\"{$NZBfinderLogoImage}\" >";
									}										
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$myfulltitle = str_replace("."," ",$myfulltitle);
$myfulltitle = wordwrap($myfulltitle, 12, " ");
				...
				$mytrimtitle = wordwrap($mytrimtitle, 12, " ");
			...
			//	$attrList = $row["attrList"];		
				...
				$inline_content = "hrnzbshiddencontent{$itemNr}";
									...
									$itemCnt1a="<br><hr><form action='JavaScript:getTrailersdotorg($myimdb)' method='POST' id='dotorg{$myimdb}' >
														...
														<div id='mytrailersnzbcat'></div><hr><font color=#8A8A7B>{$myfulltitle}</font> </font>";
			...
			//	echo "<br>$img<br>";				
				...
				echo "<div style=\" padding-left: 10px; \"><font id=bubble class=bubble style=\"color: #888888; font-size: 9pt; \">Feed database is empty at the moment. It is possible the site is down.<br>
					<a href={$NZBcatLandingPage} target=hrnew title=\"Check the Site\"><img src=includes/images/crossbones.png height=60 style=\" opacity: 0.4;  \"></a></font></div>"; 
			}		
		?>
						</div></div>
