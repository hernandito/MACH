...
function nzbndxRefresh2x(){
$("#nzbndxdiv").load("blank.php");
$("#nzbndxdiv").load("parse_nzbndx2db.php");
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    {
		$("#mytrailersnzbndx").innerHTML="Please wait....";
        $("#mytrailersnzbndx").load("trailersearch.php",$("#dotorg" + movieid).serialize()); 
    }
	
    function addmovietomycart6(movieid2)
    {
		$("#mytrailersnzbndx").innerHTML="Please wait....";
        $("#mytrailersnzbndx").load("addtocart.php",$("#nzbndx" + movieid2).serialize()); 
    }	
					...
					<tr valign="top">  					
						<td width=30px bgcolor=white height=76>	
							<?php
									...
									$timeresult = mysql_query('SELECT timestamp from nzbndx');		
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM nzbsdotorg";	dog								
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM nzbndx";
									$result = $conn->query($sql);											
									if ($result->num_rows > 0) {
										echo "<a href={$NZBNDXLandingPage} target=hrnew><img style=\"margin-top:5px; width: 28px; margin-left:3px;\" src=\"{$NZBNDXLogoImage}\" >";
									}										
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$addmycart = "http://nzbndx.com/api?t=cartadd&id={$myguid}&apikey=be95c37bb893582ba80eaec60086c12f";
			...
			//	$attrList = $row["attrList"];		
				...
				$addtocartbutton = "<form action='JavaScript:addmovietomycart6($myimdb)' method='POST' id='nzbndx{$myimdb}' >
											<input type='hidden' id='moviecart' name='moviecart' value='{$addmycart}'>
											<input type='hidden' id='movieid2' name='movieid2' value='{$myimdb}'>
											<input type='hidden' id='moviename2' name='moviename2' value='{$mytrimtitle}'>
				...
				$inline_content = "hrnzbndxhiddencontent{$itemNr}";
									...
									$itemCnt1a="<br><hr><form action='JavaScript:getTrailersdotorg($myimdb)' method='POST' id='dotorg{$myimdb}' >
														...
														<div id='mytrailersnzbndx'></div><hr><font color=#8A8A7B>{$myfulltitle}</font> </font>";
			...
			if (stripos($mytrimtitle,'one piece') !== false) {
			...
			}	elseif (stripos($mytrimtitle,'daily show') !== false) {	
			}	elseif (stripos($mytrimtitle,'seth meyers') !== false) {	
			}	elseif (stripos($mytrimtitle,'rachel maddow') !== false) {	
			}	elseif (stripos($mytrimtitle,'sunday without god') !== false) {				
			}	elseif (stripos($mytrimtitle,'david letterman') !== false) {	
			}	elseif (stripos($mytrimtitle,'voetbal') !== false) {	
			}	elseif (stripos($mytrimtitle,'ellen degeneres') !== false) {	
			}	elseif (stripos($mytrimtitle,'jimmy fallon') !== false) {			
			}	elseif (stripos($mytrimtitle,'jimmy kimmel') !== false) {
			} else {
				...
				echo "<div style=\" padding-left: 10px; \"><font id=bubble class=bubble style=\"color: #888888; font-size: 9pt; \">Feed database is empty at the moment. It is possible the site is down.<br>
					<a href={$NZBNDXLandingPage} target=hrnew title=\"Check the Site\"><img src=includes/images/crossbones.png height=60 style=\" opacity: 0.4;  \"></a></font></div>"; 
			}		
		?>
						</div>
					...
					<tr>
							<td  valign=top bgcolor=white >
								<center><a  href="#" title="Refresh" onClick="nzbtvRefresh2x()" ><img src="includes/images/refresh.png"></a>
								<br>
								<?php
									if ($mytimestamp != "") { 								
