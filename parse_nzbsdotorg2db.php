    ...
    function addmovietomycart(movieid2)
    {
						...
						<td width=30px bgcolor=white height=76>	
							<?php
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM nzbsdotorg";	dog								
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM nzbsdotorg";
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				$hrmovietitle2 = wordwrap($hrmovietitle2, 13, "\n", true);				

				$addmycart = "http://nzbs.org/api?t=cartadd&id={$myguid}&apikey=00bfbf2569b57394a756849a9499ad2c";
			...
			//	$attrList = $row["attrList"];		
				...
				$addtocartbutton = "<form action='JavaScript:addmovietomycart($myimdb)' method='POST' id='dotorg2{$myimdb}' >
									...
									$itemCnt1a = 	"<br><hr>
												...
												{$myimdblink}<div id='mytrailersdotorg'></div><hr><font color=#8A8A7B style='font-size:11px'>{$myfulltitle}</font> </font>";
			...
			if (stripos($mytrimtitle,'one piece') !== false) {
			...
			}	elseif (stripos($mytrimtitle,'eastenders') !== false) {
			} else {				
				echo $itemCnt;																	  
				...
				echo "<div style=\" padding-left: 10px; \"><font id=bubble class=bubble style=\"color: #888888; font-size: 9pt; \">Feed database is empty at the moment. It is possible the site is down.<br>
					<a href={$NZBdotorgLandingPage} target=hrnew title=\"Check the Site\"><img src=includes/images/crossbones.png height=60 style=\" opacity: 0.4;  \"></a></font></div>"; 
			}
							?>
						</div>
