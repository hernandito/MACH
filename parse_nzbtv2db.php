...
$("#nzbtvdiv").load("blank.php");
$("#nzbtvdiv").load("parse_nzbtv2db.php");
...
<SCRIPT LANGUAGE="JavaScript">
    ...
    {
		$("#mytrailersnzbtv").innerHTML="Please wait....";
        $("#mytrailersnzbtv").load("trailersearch.php",$("#dotorg" + movieid).serialize()); 
    }
	
    function addmovietomycart2(movieid2)
    {
		$("#mytrailersnzbtv").innerHTML="Please wait....";
        $("#mytrailersnzbtv").load("addtocart.php",$("#nzbtv" + movieid2).serialize()); 
    }	
					...
					<tr valign="top">  					
						<td width=30px bgcolor=white height=76>	
							<?php
									...
									$timeresult = mysql_query('SELECT timestamp from nzbtv');		
								...
								//	$sql = "SELECT timestamp, mytrimtitle, descriptions, myimdbtt, myimdb, myfulltitle, mylink, img, items, attrList, myitemCnt FROM nzbsdotorg";	dog								
									$sql = "SELECT timestamp, mytrimtitle, myimdbtt, myimdb, myfulltitle, mylink, img, itemdetails, guid FROM nzbtv";
									$result = $conn->query($sql);											
									if ($result->num_rows > 0) {
										echo "<a href={$NzbTVLandingPage} target=hrnew><img style=\"margin-top:5px; margin-left:3px;\" src=\"{$NzbTVLogoImage}\" >";
									}										
			...
			//	$attrList = $row["attrList"];		
				...
				$inline_content = "hrnzbshiddencontent{$itemNr}";
									...
									$itemCnt1a="<br><hr><form action='JavaScript:getTrailersdotorg($myimdb)' method='POST' id='dotorg{$myimdb}' >
														...
														<div id='mytrailersnzbtv'></div><hr><font color=#8A8A7B>{$myfulltitle}</font> </font>";
...
if (stripos($mytrimtitle,'one piece') !== false) {
				...
				echo "<div style=\" padding-left: 10px; \"><font id=bubble class=bubble style=\"color: #888888; font-size: 9pt; \">Feed database is empty at the moment. It is possible the site is down.<br>
					<a href={$NzbTVLandingPage} target=hrnew title=\"Check the Site\"><img src=includes/images/crossbones.png height=60 style=\" opacity: 0.4;  \"></a></font></div>"; 
			}		
		?>
						</div>
